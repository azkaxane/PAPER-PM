...
# =====================================================================
# SIDEBAR — INPUT DATA LENGKAP DENGAN TOOLTIPS
# =====================================================================
with st.sidebar:
...
if fit_success:
    ...
    # =================================================================
    # LAYOUT 4 TAB (LENGKAP)
    # =================================================================
    tab1, tab2, tab3, tab4 = st.tabs([
        "📊 Profil Kesehatan Mesin", 
        "💰 Optimasi & Rekomendasi Jadwal",
        "🌱 Analisis Sensitivitas (Uji Skenario)", 
        "📑 Ringkasan & Cetak PDF"
    ])
    ...
    with tab1:
        ...
        <strong>Validitas Data (Uji Statistik):</strong> p-value = {p_value:.4f} — 
    ...
    with tab2:
        ...
        with colx:
            st.markdown("##### 🏢 Strategi Lama (Hanya Fokus Penghematan Uang)")
        ...
        # Limit sumbu Y untuk membuang anomali infinity di jam awal
    ...
    with tab4:
        ...
        st.caption("Cetak laporan PDF lengkap berisi parameter teknis, kurva, dan rekomendasi keputusan untuk diserahkan ke manajemen atau lampiran akademik.")

        # ---------------- Generate PDF (Tetap Lengkap seperti kode awal) ----------------
        def generate_pdf():
            buffer = io.BytesIO()
            ...
            story.append(Paragraph("2. Kesimpulan Jadwal yang Direkomendasikan", heading_style))
            rekomendasi_text = f"""
            ...
            Namun, setelah menyeimbangkan faktor kerugian uang dan komitmen lingkungan (dengan bobot {w:.2f}), jadwal paling optimal berubah menjadi <b>{T_optimal_sust:.0f} jam</b>. 
            Pada siklus ini, perusahaan dapat menjaga efisiensi sekaligus menahan laju emisi di angka {emisi_at_sust:.4f} kg CO2 per jam operasi.
            """
