...
if fit_success:
    ...
    # -----------------------------------------------------------------
    # TAB 4: LAPORAN & CETAK PDF (VERSI TERBARU & MENARIK)
    # -----------------------------------------------------------------
    with tab4:
        ...
        def generate_pdf():
            ...
            # Style untuk Heading/Judul Bab (Background Biru, Teks Putih, Padding)
            ...
            # Pembuatan Tabel di PDF dengan styling warna-warni
            ...
            # Konversi figure Plotly (Siklus Hidup) ke PNG dengan resolusi tinggi (scale=2)
            ...
            # Pindah Halaman agar grafik selanjutnya tidak terpotong
            story.append(PageBreak()) 

            # ================= BAGIAN 4: GRAFIK OPTIMASI BIAYA & EMISI =================
            story.append(Paragraph("4. Kurva Trade-off: Biaya Operasional vs Emisi Karbon", heading_style))
            # Konversi figure Plotly (Trade-off) ke PNG
            ...
            story.append(Paragraph(f"Analisis Diagram: Kurva ini menunjukkan benturan antara biaya finansial (garis biru solid) dan denda/emisi lingkungan (garis hijau putus-putus). Titik hijau vertikal di {T_optimal_sust:.0f} Jam adalah jalan tengah terbaik (optimal) bagi perusahaan.", caption_style))
            story.append(Spacer(1, 10))

            # ================= BAGIAN 5: GRAFIK SENSITIVITAS =================
            story.append(Paragraph("5. Analisis Sensitivitas Perubahan Kebijakan", heading_style))
            # Konversi figure Plotly (Sensitivitas w) ke PNG
            ...
            story.append(Paragraph("Analisis Diagram: Simulasi pergeseran rekomendasi jadwal jika pihak manajemen memutuskan untuk lebih condong pada penghematan uang (titik mendekati 0) atau lebih condong pada penekanan polusi (titik mendekati 1).", caption_style))
            
            # Membangun/Compile Dokumen PDF
            doc.build(story)
            buffer.seek(0)
            return buffer
        ...
        try:
            ...
            st.download_button(
                label="📥 Unduh Laporan Eksekutif (PDF Kualitas Tinggi)",
                data=pdf_buffer,
                file_name="Laporan_Sustainable_PM_Update.pdf",
                mime="application/pdf",
                use_container_width=True,
                type="primary"
            )
        except Exception as e:
            st.error(f"Gagal membuat PDF. Pastikan library 'kaleido' telah terinstal: pip install -U kaleido. Error: {e}")
