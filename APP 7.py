...
# Menyuntikkan Palet Cosmic Sunset
st.markdown("""
<style>
    /* Mengatur Background Utama ke Warna 8 dan Teks ke Warna 5 */
    .stApp { background-color: #1A1C29; color: #9CA3AF; }
    
    /* Header & Teks */
    .main-header { font-size: 2.2rem; font-weight: 700; color: #FFA3E5; margin-bottom: 0; } /* Warna 1 */
    .sub-header { font-size: 1.1rem; color: #A6B1F7; margin-top: 5px; margin-bottom: 20px; } /* Warna 2 */
    .section-title { font-size: 1.3rem; font-weight: 600; color: #F48A42; border-left: 4px solid #EC3D6D; padding-left: 10px; margin-top: 1.5rem; margin-bottom: 1rem; } /* Warna 4 & 3 */
    
    /* Box Informasi & Peringatan */
    .info-box { background-color: #292965; border-left: 4px solid #A6B1F7; padding: 15px; border-radius: 6px; font-size: 0.95rem; margin-bottom: 15px; color: #FFFFFF; } /* Warna 7 & 2 */
    .warning-box { background-color: #4B5162; border-left: 4px solid #F48A42; padding: 15px; border-radius: 6px; font-size: 0.95rem; margin-bottom: 15px; color: #FFFFFF; } /* Warna 6 & 4 */
    
    /* Modifikasi Angka (Metrics) */
    div[data-testid="stMetricValue"] { font-size: 1.6rem; color: #EC3D6D; font-weight: bold; } /* Warna 3 */
    div[data-testid="stMetricLabel"] { color: #A6B1F7 !important; } /* Warna 2 */
</style>
...
if fit_success:
    ...
    pohon_setara = max(0, emisi_saved / 22.0)
    ...
    with tab1:
        ...
        with st.expander("🔍 Lihat Plot Probabilitas Weibull (Uji Validitas Akademik)"):
            ...
            fig_prob = go.Figure()
            # Warna 3 (Pink Terang) untuk titik, Warna 2 (Periwinkle) untuk garis
    ...
    with tab2:
        ...
        # Warna 2 (Biru Terang) dan Warna 4 (Oranye)
        fig2.add_trace(go.Scatter(x=T_curve, y=cost_curve, name="Biaya Operasional (Rp)", line=dict(color='#A6B1F7', width=3)), secondary_y=False)
        fig2.add_trace(go.Scatter(x=T_curve, y=emisi_cost_curve, name="Biaya Polusi Emisi (Rp)", line=dict(color='#F48A42', width=3, dash='dot')), secondary_y=True)
        fig2.add_vline(x=T_optimal_conv, line_dash="dash", line_color="#A6B1F7", annotation_text=f"Jadwal Lama: {T_optimal_conv:.0f} Jam")
        fig2.add_vline(x=T_optimal_sust, line_dash="dash", line_color="#F48A42", annotation_text=f"Jadwal Baru: {T_optimal_sust:.0f} Jam")
        fig2.update_layout(template="plotly_dark", paper_bgcolor='#1A1C29', plot_bgcolor='#1A1C29', height=420, legend=dict(orientation="h", y=1.15), margin=dict(t=60, b=30), yaxis_range=[min(cost_curve)*0.9, min(cost_curve)*3])
        st.plotly_chart(fig2, use_container_width=True)

        st.markdown('<p class="section-title">Skor Gabungan (0 Berarti Paling Sempurna)</p>', unsafe_allow_html=True)
        fig3 = go.Figure()
        # Warna 3 (Pink Terang)
        fig3.add_trace(go.Scatter(x=T_curve, y=norm_curve, name="Skor Penilaian Gabungan", line=dict(color='#EC3D6D', width=3), fill='tozeroy'))
        fig3.add_vline(x=T_optimal_sust, line_dash="dash", line_color="#EC3D6D", annotation_text=f"Jadwal Paling Sempurna = {T_optimal_sust:.0f} Jam")
        fig3.update_layout(template="plotly_dark", paper_bgcolor='#1A1C29', plot_bgcolor='#1A1C29', height=350, margin=dict(t=30, b=30), xaxis_title="Pilihan Jadwal Servis (Jam)", yaxis_title="Skor Penilaian (Makin kecil makin baik)")
    ...
    with tab3:
        ...
        fig4.update_layout(template="plotly_dark", paper_bgcolor='#1A1C29', plot_bgcolor='#1A1C29', height=350, margin=dict(t=30, b=30), xaxis_title="Bobot Kepedulian Lingkungan (0.0 sampai 1.0)", yaxis_title="Rekomendasi Jadwal (Jam)")
        ...
        # PERBAIKAN LOGIKA: Mengunci bounds dari kondisi baseline (Kondisi Saat Ini)
    ...
    with tab4:
        ...
        def generate_pdf():
            ...
            # Menyesuaikan warna judul PDF dengan palet
            ...
            story.append(Paragraph(f"Analisis Diagram: Kurva ini menunjukkan benturan antara biaya finansial dan denda/emisi lingkungan. Titik vertikal di {T_optimal_sust:.0f} Jam adalah jalan tengah terbaik bagi perusahaan.", caption_style))
            story.append(Spacer(1, 10))
