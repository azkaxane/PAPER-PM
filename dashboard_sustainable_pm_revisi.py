...
with st.sidebar:
    ...
    if input_mode == "Upload CSV":
        ...
        manual_text = st.text_area(
            ...
            value="345.4, 775, 541, 461.4, 221.9, 221.9, 141.1, 650.4, 462.7, 525.4, 89.1, 827.9, 617.7, 257.5, 238.8, 239.8, 308.9, 422.1, 374.6, 301.9"
        )
        try:
    ...
    if data_is_synthetic:
        st.warning("⚠️ Data yang aktif adalah **data contoh (sintetis)**, dibangkitkan secara acak. "
                    "Jangan sajikan hasil dari data ini sebagai temuan studi kasus nyata.")
...
if fit_success:
    ...
    # Referensi run-to-failure untuk narasi penghematan (lebih meyakinkan daripada vs jadwal lama)
    ...
    with tab1:
        if data_is_synthetic:
            st.markdown('<div class="danger-box">⚠️ <strong>Data yang ditampilkan adalah data contoh (SINTETIS)</strong>, '
                         'bukan data historis kerusakan mesin yang sebenarnya. Gunakan hanya untuk demonstrasi '
                         'fungsi dashboard.</div>', unsafe_allow_html=True)
    ...
    with tab2:
        ...
        st.caption("Baseline yang relevan bukan hanya kebijakan cost-optimal, tetapi juga jadwal aktual "
                   "perusahaan (jika tersedia) dan run-to-failure — karena keduanya mencerminkan kondisi "
                   "nyata sebelum optimasi diterapkan.")
        st.dataframe(df_policy_display, use_container_width=True)

        if cost_change_rp > 0:
            st.markdown(f"""
            <div class="danger-box">
            📌 <strong>Perhatian:</strong> Dibandingkan kebijakan cost-optimal murni, kebijakan sustainable-optimal
            justru menaikkan biaya tahunan sebesar <b>+Rp {cost_change_rp/1e6:,.1f} juta ({cost_change_pct:+.1f}%)</b>.
            Ini adalah trade-off yang wajar ketika bobot lingkungan (w) diberikan, namun harus dinyatakan secara
            jujur, bukan disajikan sebagai penghematan.
            </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
        ...
        st.markdown(f"""
        <div class="info-box">
        💡 <strong>Perbandingan yang lebih meyakinkan (vs Run-to-Failure):</strong> Dibandingkan kebijakan
        tanpa PM sama sekali (run-to-failure, ≈Rp {annual_cost_rtf/1e6:,.1f} juta/tahun), kebijakan
        sustainable-optimal menghemat sekitar <b>Rp {savings_vs_rtf_rp/1e6:,.1f} juta/tahun
        ({savings_vs_rtf_pct:.1f}%)</b>. Perbandingan ini lebih relevan karena run-to-failure/jadwal informal
        mencerminkan praktik yang lebih dekat dengan kondisi sebelum penelitian ini dilakukan.
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab3:
        ...
        st.caption("Grafik ini menggantikan 'skor gabungan' sebelumnya karena lebih transparan: "
                   "setiap titik pada kurva adalah pilihan interval T tertentu, dengan sumbu X = biaya "
                   "tahunan dan sumbu Y = emisi tahunan. Tidak ada normalisasi tersembunyi.")
        ...
        if band_mask.sum() > 1:
            ...
            pada rentang tersebut berkisar {emisi_band_low:,.0f}–{emisi_band_high:,.0f} kg CO2. Artinya, bergeser
            ke arah interval yang lebih rendah emisi dalam rentang ini <b>hampir tidak menambah biaya</b> —
            pesan ini secara empiris lebih kuat untuk mendukung argumen keberlanjutan dibanding angka absolut
            reduksi emisi yang kecil.
            </div>
            """, unsafe_allow_html=True)
    ...
    with tab4:
        ...
        studi ini. Keterbatasan ini dinyatakan secara eksplisit dan direkomendasikan sebagai arah penelitian
        lanjutan, bukan diimplementasikan secara tidak lengkap pada versi ini.
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab5:
        ...
        if data_is_synthetic:
            st.warning("⚠️ Laporan ini akan mencantumkan bahwa data yang digunakan adalah data contoh (sintetis).")
